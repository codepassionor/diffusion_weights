    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            for layer_tag in pixel_value:
                inputs_list = [pixel_value[layer_tag][:, :, :768], pixel_value[layer_tag][:, :, 768:1536], pixel_value[layer_tag][:, :, 1536:2304], pixel_value[layer_tag][:, :, 2304:]]
                target_list = [targets[layer_tag][:, :, :768], targets[layer_tag][:, :, 768:1536], targets[layer_tag][:, :, 1536:2304], targets[layer_tag][:, :, 2304:]]
                for i in range(len(inputs_list)):
                    inputs = inputs_list[i]
                    target = target_list[i]    
                    with torch.no_grad():
                        inputs = inputs.unsqueeze(0).reshape(1, 3, -1, 256)
                        target = target.unsqueeze(0).reshape(1, 3, -1, 256)
                        inputs_latent = pipeline.vae.encode(inputs).latent_dist.sample()
                        #target_latent = pipeline.vae.encode(target).latent_dist.sample()
                        inputs_latent = inputs_latent * pipeline.vae.config.scaling_factor
                        #target_latent = target_latent * pipeline.vae.config.scaling_factor
                    ...
                    text = f"Fully connected layer parameters for layer {layer_tag} mlp of the GPT2 base model"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./stable-diffusion-2-1-finetuned-base", help="Directory to save the model")
    ...
    parser.add_argument("--data_dir", type=str, default="/root/autodl-tmp/llm_weight/param_data/param_dict_gpt2.pt", help="Path to the directory containing images")
