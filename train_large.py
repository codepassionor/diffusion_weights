...
import argparse
from my_datasets import ParaDataset_large

def chunk(x):
    h_chunks = torch.split(x, 640, dim=1)
    all_chunks = []
    for hc in h_chunks:
        w_chunks = torch.split(hc, 320, dim=2)
...
def main(args):
    ...
    dataset = ParaDataset_large(args.data_dir)
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            for layer_tag in pixel_value:
                inputs_list = chunk(pixel_value[layer_tag])
                target_list = chunk(targets[layer_tag])
                for i in range(len(inputs_list)):
                    ...
                    text = f"Fully connected layer parameters for layer {layer_tag} mlp of the GPT2 large model"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./stable-diffusion-2-1-finetuned-large", help="Directory to save the model")
    ...
    parser.add_argument("--data_dir", type=str, default="/root/autodl-tmp/llm_weight/param_data/param_dict_gpt2_large", help="Path to the directory containing images")
